...
class CalendarData:
    ...
    def __init__(
        self, data_folder: str, first_weekday: int = constants.WEEK_START_DAY_MONDAY
    ) -> None:
    ...
    def load_calendar(self, filename: str) -> Dict:
        with open(
            os.path.join(".", self.data_folder, "{}.json".format(filename))
        ) as file:
    ...
    def users_list(
        self, data: Optional[Dict] = None, calendar_id: Optional[str] = None
    ) -> List:
        if data is None:
    ...
    def task_from_calendar(
        self, calendar_id: str, year: int, month: int, day: int, task_id: int
    ) -> Dict:
        data = self.load_calendar(calendar_id)
    ...
    def repetitive_task_from_calendar(
        self, calendar_id: str, year: int, month: int, task_id: int
    ) -> Dict:
        data = self.load_calendar(calendar_id)
    ...
    def add_repetitive_tasks_from_calendar(
        self, year: int, month: int, data: Dict, tasks: Dict
    ) -> Dict:
        (
            current_day,
            current_month,
    ...
    def delete_task(
        ...
        if (
            ...
            for index, task in enumerate(
                ...
                if task["id"] == task_id:
                    data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str][day_str].pop(
                        index
                    )
                    deleted = True
    ...
    def update_task_day(
        ...
        for index, task in enumerate(
            ...
            if task["id"] == task_id:
                task_to_update = data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str][
                    day_str
                ].pop(index)
        ...
        data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str][new_day_str].append(
            task_to_update
        )
    ...
    def create_task(
        ...
        if has_repetition:
            if (
                repetition_type == self.REPETITION_SUBTYPE_MONTH_DAY
                and repetition_value == 0
            ):
                return False
            ...
            if day_str not in data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str]:
                data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str][day_str] = []
            data[KEY_TASKS][KEY_NORMAL_TASK][year_str][month_str][day_str].append(
                new_task
            )
    ...
    def hide_repetition_task_instance(
        ...
        if (
            month_str
            not in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][task_id_str][year_str]
        ):
            data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][task_id_str][year_str][
                month_str
            ] = {}
    ...
    def add_task_to_list(
        tasks: Dict, day_str: str, month_str: str, new_task: Dict
    ) -> None:
    ...
    def _repetitive_tasks_from_calendar(
        self, year: int, month: int, data: Dict
    ) -> Dict:
        ...
        year_and_months = set(
            [
                (source_day.year, source_day.month)
                for source_day in self.gregorian_calendar.month_days(year, month)
            ]
        )
        ...
        for source_year, source_month in year_and_months:
            ...
            for task in data[KEY_TASKS][KEY_REPETITIVE_TASK]:
                ...
                for week in self.gregorian_calendar.month_days_with_weekday(
                    ...
                    for weekday, day in enumerate(week):
                        ...
                        if (
                            ...
                            and not self._is_repetition_hidden_for_day(
                                data, id_str, year_str, month_str, str(day)
                            )
                            and task["repetition_value"] == weekday
                        ):
                            self.add_task_to_list(
                                repetitive_tasks, day_str, month_str, task
                            )
                        elif task[
                            "repetition_type"
                        ] == self.REPETITION_TYPE_MONTHLY and not self._is_repetition_hidden(
                            data, id_str, year_str, month_str
                        ):
                            if (
                                task["repetition_subtype"]
                                == self.REPETITION_SUBTYPE_WEEK_DAY
                            ):
                                if (
                                    task["repetition_value"] == weekday
                                    and not monthly_task_assigned
                                ):
    ...
    @staticmethod
    def _is_repetition_hidden_for_day(
        data: Dict, id_str: str, year_str: str, month_str: str, day_str: str
    ) -> bool:
        if id_str in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK]:
            if (
                year_str in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][id_str]
                and month_str
                in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][id_str][year_str]
                and day_str
                in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][id_str][year_str][
                    month_str
                ]
    ...
    @staticmethod
    def _is_repetition_hidden(
        data: Dict, id_str: str, year_str: str, month_str: str
    ) -> bool:
        if id_str in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK]:
            if (
                year_str in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][id_str]
    ...
    def _save_calendar(self, data: Dict, filename: str) -> None:
        ...
        with open(
            os.path.join(".", self.data_folder, "{}.json".format(filename)), "w+"
        ) as file:
            json.dump(data, file)
        ...
        for task_id in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK]:
            for year in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][task_id]:
                for month in data[KEY_TASKS][KEY_REPETITIVE_HIDDEN_TASK][task_id][year]:
                    ...
                    if (current_date - task_date).days > current_app.config[
